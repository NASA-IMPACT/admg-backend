...
class InstrumentTest(TestCase):
    ...
    def test_create_instrument_instance(self):
        ...
        self.assertEqual(Change.objects.filter(content_type=self.content_type).count(), 0)
        self.client.force_login(user=self.user)
        # making the post request to send the test data through model forms
        self.client.post(
            self.url,
            {
                "content_type": self.content_type.id,
        ...
        instrument = Change.objects.get(content_type=self.content_type)
        # assert the additional metadata values
        self.assertEqual(instrument.update['additional_metadata'], {'testkey': 'testvalue'})
